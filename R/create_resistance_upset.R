#' Create Upset Plots for Resistance Determinants, Faceted by Isolate Type
#'
...
#'   `Isolate_type`, `resistance_score`, and the resistance factor columns defined
#'   within the function (`Bla_inhR_acquired`, `Bla_ESBL_acquired`, etc.).
...
#'
#' @examples
#' # create_resistance_upset_plots(data = my_data, output_dir = "plots", legend_size = 12)
create_resistance_upset_plots <- function(data, output_dir, legend_size = 12, label_size = legend_size) {
  resistance_classes <- c(
    "Bla_acquired",
    "Bla_inhR_acquired",
    "Bla_ESBL_acquired",
  ...
  required_cols <- c("Isolate_type", "resistance_score", resistance_classes)
  ...
  data_converted <- data |>
    dplyr::mutate(
      dplyr::across(
        dplyr::all_of(resistance_classes),
        ~ !is.na(.) & . != "-"
      )
    )
    ...
    upset_plot <- ComplexUpset::upset(
      ...
      base_annotations = list(
        "Frequency" = ComplexUpset::intersection_size(
          ggplot2::aes(fill = as.factor(.data$resistance_score))
        ) +
